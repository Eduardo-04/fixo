...
export default function CategoryCard({ category, count }: CategoryCardProps) {
  ...
  return (
    <Link
      ...
      className="group relative bg-white dark:bg-slate-800/80 rounded-2xl p-5 border border-slate-200 dark:border-slate-700 shadow-card hover:shadow-card-hover hover:border-brand-primary/40 dark:hover:border-brand-primary/40 hover:-translate-y-1 transition-all duration-300 flex flex-col items-start text-left"
    >
      ...
      <h3 className="text-base font-bold text-slate-900 dark:text-slate-100 group-hover:text-brand-primary dark:group-hover:text-brand-primary transition-colors line-clamp-1">
        {category.name}
      </h3>
      ...
      <div className="mt-4 pt-3 border-t border-slate-100 dark:border-slate-700/50 w-full flex items-center justify-between text-xs text-slate-400">
        ...
        {count !== undefined && (
          <span className="bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 px-2 py-0.5 rounded-full text-[11px] font-semibold">
            {count}
          </span>
        )}
      </div>
