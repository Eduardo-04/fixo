...
export default function TechnicianCard({ technician, className = '' }: TechnicianCardProps) {
  ...
  const categoryNames = technician.categories?.map((c) => c.name).join(', ') || 'Servicios Técnicos';

  return (
    <div
      className={`group relative bg-white rounded-2xl border border-slate-200 p-5 shadow-card hover:shadow-card-hover hover:border-slate-300 transition-all duration-300 flex flex-col justify-between ${
        technician.is_pro ? 'ring-2 ring-brand-primary/20 bg-gradient-to-b from-orange-50/20 to-white' : ''
      } ${className}`}
    >
      <div>
        ...
        <div className="flex items-start gap-4 mb-3.5">
          ...
          <div className="flex-1 min-w-0">
            ...
            <Link href={`/t/${technician.slug}`}>
              <h3 className="text-base font-bold text-slate-900 hover:text-brand-primary transition-colors truncate">
      ...
      <div className="flex items-center gap-2 pt-3 border-t border-slate-100 mt-auto">
        <Link
          ...
          className="flex-1 py-2 px-3 text-center text-xs font-semibold text-slate-700 bg-slate-100 hover:bg-slate-200 rounded-xl transition-colors flex items-center justify-center gap-1"
        >
